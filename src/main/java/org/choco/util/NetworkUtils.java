...
public class NetworkUtils {
	 ...
	 * 如果它们的协议、端口（如果指明了的话）和主机名都相同。则他们属于同源。 简单说：浏览器限制脚本只能和同协议、同域名、同端口的脚本进行交互。<br>
	 * 注：
	 * 由于IE8-IE10不支持通过设置Access-Control-Allow-Origin头的方式，所以对于IE需要按照IE提供的方案使用XDomainRequest对象解决。<br>
	 ...
	 *
	 * @param type
	 *            如：request.getParameter("type"); request 代表客户端http请求信息对象
	 * @param response
	 *            代表服务器端http响应信息对象
	 * @author Tony_tian
	 */
